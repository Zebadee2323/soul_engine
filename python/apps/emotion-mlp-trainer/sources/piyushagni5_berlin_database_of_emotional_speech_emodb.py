#!/usr/bin/env python3
"""Create a source_data manifest CSV from piyushagni5/berlin-database-of-emotional-speech-emodb."""
...
DATASET_REF = "piyushagni5/berlin-database-of-emotional-speech-emodb"
...
IGNORED_FILENAME_LABELS = {
    "L": "Boredom",  # Langeweile is outside the shared six-label set.
}
LABEL_ALIASES = {
    "angry": "Anger",
    "anger": "Anger",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Download piyushagni5/berlin-database-of-emotional-speech-emodb "
            "and write a source_data CSV manifest for dataset-transform.py."
        )
    )
    parser.add_argument(
        "--dataset-ref",
    ...
    parser.add_argument(
        ...
        default=(
            app_dir
            / "source_data"
            / "piyushagni5_berlin_database_of_emotional_speech_emodb.csv"
        ),
...
def canonical_label_for_filename(path: Path) -> str | None:
    """Return the manifest emotion label encoded in an EmoDB WAV filename."""
    stem = path.stem
    if len(stem) < 6:
        return None

    emotion_code = stem[5].upper()
    if emotion_code in IGNORED_FILENAME_LABELS:
        return None
    return FILENAME_LABELS.get(emotion_code)
...
def canonical_label_for_wav(path: Path, dataset_root: Path) -> str | None:
    """Return the shared manifest emotion label for a WAV, ignoring unsupported emotions."""
    filename_label = canonical_label_for_filename(path)
    if filename_label is not None:
        return filename_label
    return canonical_label_for_path(path, dataset_root)
...
def collect_wav_paths(
    ...
    for wav_path in sorted(dataset_root.rglob("*.wav")):
        label = canonical_label_for_wav(wav_path, dataset_root)
    ...
    if missing_labels:
        raise RuntimeError(
            "No WAV files were found for EmoDB emotion code(s): "
            + ", ".join(missing_labels)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # CLI should report failures without traceback by default.
        print(
            "piyushagni5-berlin-database-of-emotional-speech-emodb-source: "
            f"{error}",
            file=sys.stderr,
        )
        return 1
